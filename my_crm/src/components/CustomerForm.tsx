...
type CustomerPayload = { name: string; email?: string; phone?: string; company?: string; address?: string }
...
export default function CustomerForm({
  ...
  onSave: (payload: CustomerPayload & { materials?: number[] }) => void | Promise<void>
  onCancel?: () => void
}) {
  ...
  return (
    <form onSubmit={submit} className="p-4 bg-white rounded shadow-sm">
      ...
      <div className="mb-2">
        ...
        <div>
          ...
          <div className="mb-2 text-sm">
            Ausgewählt: {materials.length} {materials.length === 1 ? 'Material' : 'Materialien'}
          </div>
      ...
      <div className="flex items-center space-x-2 mt-3">
        <button type="submit" className="px-3 py-2 btn-primary rounded">
          Speichern
        </button>
        {onCancel && (
